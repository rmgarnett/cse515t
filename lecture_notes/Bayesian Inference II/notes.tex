...
interval $(0, 1)$ -- in other words, a beta prior with $\alpha = \beta = 1$:
\[
...
\[
  ...
  0.
\]
The posterior probability of the coin being \emph{exactly} fair is zero!
Although this might seem surprising at first glance, it is a reasonable
conclusion, as the idea that a coin could be \emph{perfectly} fair is somewhat
absurd, as is suggesting that we could possibly know the bias of a coin with
infinite precision after making a finite number of observations.
...
\]
Here $\mc{H}(\varepsilon)$ represents the hypothesis of the coin being
``$\varepsilon$-close'' to fair.  We may then compute the posterior probability
of these hypotheses and consider how they vary as a function of
$\varepsilon$. Figure \ref{near_fair_probabilities} shows the results for the
coin-flipping example above. We can see that there's approximately a 50\%
posterior probability that the bias of the coin is in the interval $(0.4, 0.6)$,
corresponding to $\varepsilon = 0.1$. We also have evidence to conclude $\theta
\in (0.25, 0.75)$ with near certainty. These probabilities help quantify
exactly how ``fair'' or ``not fair'' we believe the coin to be in light of our
evidence.
...
In the next lecture we will consider \emph{point estimation,} which is one
common summarization method. Another commonly considered problem is
\emph{interval summarization,} where we provide an interval $(\ell, u)$
indicating plausible values of the parameter $\theta$ in light of the observed
data. Classical interval estimates are known as \emph{confidence intervals,} and
we will discuss them in more detail shortly.
...
\emph{confidence interval} for the parameter of interest $\theta$. Recall that
for any given dataset $\data$ and interval $(\ell, u)$, the (frequency)
probability of $\theta \in (\ell, u)$ is either 0 or 1, which is not (yet)
terribly useful for our goal.

We can resolve this tension by embracing the frequency definition of probability
and reason about the (frequency) probability of success when repeating a
particular experiment infinitely many times. The experiment we consider will
proceed as follows. Suppose we have some (for now, arbitrary) procedure
$\ci(\data)$ that maps a given dataset $\data$ to an interval $(\ell, u) =
\ci(\data)$. Now we consider repeating the following experiment:
\begin{itemize}
\item
  collect data $\data$
\item
  compute the interval $(\ell, u) = \ci(\data)$ using the procedure
\item
  state $\theta \in (\ell, u)$.
\end{itemize}
...
first step. All we know is that if we apply the confidence interval procedure to
\emph{infinitely many datasets,} that it will succeed with probability
$\alpha$. However, we usually have only one \emph{particular} dataset in front
of us to analyze that we care about! We cannot say anything about the confidence
interval produced for this dataset in isolation -- the (frequency) probability
of its validity is again either 0 or 1.

Here is a simple example that shows how \eqref{wrong} can fail. Suppose we will
observe two values $x_1, x_2 \in \R$ generated independently from some unknown
distribution $p(x)$ and wish to construct a confidence interval for the mean of
the distribution generating the data, $\theta = \E[x]$. Consider the following
procedure:
\[
  \ci(\data) =
  \begin{cases}
...
and otherwise this probability is 0\%. In no case is this posterior probability
equal to the confidence level.

Another fallacy in the interpretation of confidence intervals is the so called
\emph{precision fallacy,} the supposition that shorter confidence intervals
indicate the data provide more precise information about $\theta$. A striking
illustration of this fallacy is provided by the ``lost submarine'' example
provided by Morey, et al.\ in the reference given below. I encourage you to read
this paper and reflect!

(The tl;dr is that we can generate confidence interval procedures where
precision is \emph{inversely} proportional to the width of the intervals.
However, this disconnect cannot exist with Bayesian credible intervals.)

\section*{Caveat to the caveats}

In a course like this, the instructor will usually repeatedly point out that a
$p$-value is \emph{not} the probability that the null hypothesis is true and
that an isolated 95\% confidence interval does \emph{not} contain the parameter
of interest with 95\% probability. I am obviously guilty as charged.

However, this disconnect is \emph{not} necessary! In fact, for many classical
procedures ($t$-tests, various ``exact'' tests, etc.), we can derive analogous
Bayesian procedures for which frequentist confidence intervals \emph{serve as
Bayesian credible intervals with the same confidence.} These procedures usually
also offer reinterpreations of various frequentist $p$-values (one-sided,
two-sided, etc.) in terms of the (Bayesian) posterior probability of natural
hypotheses. Invariably, deriving these corresponding procedures entails
selecting a ``uninformative'' -- and perhaps invalid -- ``prior'' distribution
for the parameters of interest. (For example, for some real-valued parameter
$\theta \in \R$, we might choose the flat ``prior'' $p(\theta) \propto 1$, which
doesn't normalize. Although this distribution doesn't normalize, the likelihood
normally will, so the posterior will still be well-formed.)

When we can derive such analogous procedures, it can offer a great deal of
insight into the frequentist procedure! For example, the Bayesian procedure will
include some explicit choice of (usually ``uninformative'') prior distribution
for $\theta$, which you may or may not deem reasonable in a given situation. If
you're willing to be ``Bayesian'' (even temporarily), you could swap in a new
prior and derive a more personalized procedure.
