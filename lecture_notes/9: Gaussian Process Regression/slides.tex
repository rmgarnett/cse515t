...
\sslide{Gaussian Processes}{A reimagining of regression}
...
\begin{frame}{Noisy posterior}
  ...
  \begin{equation*}
    p(\vec{f}^\ast \given \vec{x}^\ast, \data) =
    \mc{N}
    \bigl(
    \vec{f}^\ast;
    \mu_{f \given \data}(\vec{x}^\ast)
    ,
    K_{f \given \data}(\vec{x}^\ast, \vec{x}^\ast)
    \bigr),
  \end{equation*}
  where
  \begin{align*}
    ...
    \mu(x) +
    K(x, \vec{x})
    \bigl(K(\vec{x}, \vec{x}) + \maker{\sigma^2 \vec{I}}\bigr)^{-1}
    \bigl(\vec{y} - \mu(\vec{x})\bigr)
    \\
    K_{f \given \data}(x, x')
    &=
    K(x, x')
...
\begin{frame}{Hyperparameters}
  ...
  \item But this prior distribution \emphr{itself} has parameters, for
    ...
    the noise variance $\sigma_n^2$.  As parameters of a prior
